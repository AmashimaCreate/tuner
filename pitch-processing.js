...
const REFINER_OPTION_KEYS = Object.freeze([
  "maxPartials",
  "weightHarmonicCap",
  "maxPartialDriftCents",
  "minPartials",
]);

/**
 * Harmonic-weighted spectral refinement: the fine-measurement stage that sits
 * behind the tracker, in the same role as the FFT stage of commercial tuner
 * engines. Given an approximate f0 that is already trusted (the tracked
 * pitch), it measures the exact frequency of each low partial on a long
 * Hann-windowed frame — three Goertzel probes and a log-magnitude parabola
 * per partial — and combines them weighted by partial power. A guitar's low
 * partials carry far more signal-to-noise than the fundamental alone, so the
 * combined estimate is stable to well under one cent even at SNR 10 dB,
 * where the frame-by-frame tracker jitters by several cents.
 *
 * Weighting is capped at harmonic 3 and only partials 1-4 are used: real
 * strings are inharmonic (partials run progressively sharp), and leaning on
 * higher partials would bias the reading sharp by more than it gains.
 */
export class PitchRefiner {
  constructor(inputLength, options = {}) {
    for (const key of Object.keys(options)) {
      if (!REFINER_OPTION_KEYS.includes(key)) {
        throw new TypeError(`unknown refiner option: ${key}`);
      }
    }
    const {
      maxPartials = 4,
      weightHarmonicCap = 3,
      maxPartialDriftCents = 60,
      minPartials = 2,
    } = options;

    if (!Number.isInteger(inputLength) || inputLength < 16) {
      throw new TypeError("inputLength must be an integer of at least 16");
    }

    this._maxPartials = maxPartials;
    this._weightHarmonicCap = weightHarmonicCap;
    this._maxPartialDriftCents = maxPartialDriftCents;
    this._minPartials = minPartials;
    this._window = new Float32Array(inputLength);
    this._windowed = new Float32Array(inputLength);
    for (let index = 0; index < inputLength; index += 1) {
      this._window[index] = 0.5 - 0.5 * Math.cos((2 * Math.PI * index) / (inputLength - 1));
    }
  }

  /** Returns { hz, partialCount }; hz is NaN when fewer than minPartials
   *  coherent partials are found near multiples of approxHz. */
  refine(frame, sampleRate, approxHz) {
    if (frame.length !== this._window.length) {
      throw new Error(`frame must have length ${this._window.length}`);
    }
    if (!Number.isFinite(approxHz) || approxHz <= 0) {
      return { hz: Number.NaN, partialCount: 0 };
    }

    for (let index = 0; index < frame.length; index += 1) {
      this._windowed[index] = frame[index] * this._window[index];
    }
    ...
    for (let harmonic = 1; harmonic <= this._maxPartials; harmonic += 1) {
      const centerHz = approxHz * harmonic;
      if (centerHz > sampleRate * 0.45) break;

      const left = goertzelPower(this._windowed, sampleRate, centerHz - binHz);
      const center = goertzelPower(this._windowed, sampleRate, centerHz);
      const right = goertzelPower(this._windowed, sampleRate, centerHz + binHz);

      const [logLeft, logCenter, logRight] = [
        Math.log(left + 1e-30),
        Math.log(center + 1e-30),
        Math.log(right + 1e-30),
      ];
      const curvature = logLeft - 2 * logCenter + logRight;
      if (!(curvature < 0)) continue;
      const offset = (0.5 * (logLeft - logRight)) / curvature;
      if (!Number.isFinite(offset) || Math.abs(offset) > 1.5) continue;

      const impliedF0 = (centerHz + offset * binHz) / harmonic;
      if (Math.abs(centsBetween(impliedF0, approxHz)) > this._maxPartialDriftCents) continue;

      const weight = center * Math.min(harmonic, this._weightHarmonicCap);
      weightedSum += impliedF0 * weight;
      weightTotal += weight;
      partialCount += 1;
    }

    if (partialCount < this._minPartials || weightTotal <= 0) {
      return { hz: Number.NaN, partialCount };
    }
    return { hz: weightedSum / weightTotal, partialCount };
  }
}

// Exact DFT power at an arbitrary frequency.
